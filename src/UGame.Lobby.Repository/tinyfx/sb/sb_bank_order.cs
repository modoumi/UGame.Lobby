...
namespace UGame.Lobby.Repository.ing
{
	...
	public class Sb_bank_orderMO : MySqlTableMO<Sb_bank_orderEO>
	{
	    ...
	    private void RepairAddData(Sb_bank_orderEO item, bool useIgnore_, out string sql_, out List<MySqlParameter> paras_)
		{
			...
			sql_ += $" INTO {TableName} (`OrderID`, `ProviderID`, `AppID`, `OperatorID`, `UserID`, `FromMode`, `FromId`, `UserKind`, `RegistDate`, `OrderType`, `BankID`, `PaytypeID`, `PaytypeChannel`, `CurrencyID`, `RecDate`, `PlanAmount`, `AppRequestUUID`, `AppOrderId`, `AppReqComment`, `AppRequestTime`, `AccName`, `AccNumber`, `BankCode`, `IsFirstCashOfDay`, `Meta`, `Status`, `UserIP`, `OwnOrderId`, `TransMoney`, `OrderMoney`, `BankResponseTime`, `BankOrderId`, `BankCallbackTime`, `BankTime`, `BrCode`, `QrCode`, `Amount`, `OwnFee`, `UserFee`, `UserMoney`, `EndBalance`, `AmountBonus`, `EndBonus`, `SettlTable`, `SettlId`, `SettlAmount`, `SettlStatus`, `CompleteFlag`, `ActivityIds`, `IsAddBalance`, `IsAuditOrder`, `CountryID`) VALUE (@OrderID, @ProviderID, @AppID, @OperatorID, @UserID, @FromMode, @FromId, @UserKind, @RegistDate, @OrderType, @BankID, @PaytypeID, @PaytypeChannel, @CurrencyID, @RecDate, @PlanAmount, @AppRequestUUID, @AppOrderId, @AppReqComment, @AppRequestTime, @AccName, @AccNumber, @BankCode, @IsFirstCashOfDay, @Meta, @Status, @UserIP, @OwnOrderId, @TransMoney, @OrderMoney, @BankResponseTime, @BankOrderId, @BankCallbackTime, @BankTime, @BrCode, @QrCode, @Amount, @OwnFee, @UserFee, @UserMoney, @EndBalance, @AmountBonus, @EndBonus, @SettlTable, @SettlId, @SettlAmount, @SettlStatus, @CompleteFlag, @ActivityIds, @IsAddBalance, @IsAuditOrder, @CountryID);";
		...
		{
			var count = 0;
	        var insertSql = $"INSERT INTO {TableName} (`OrderID`, `ProviderID`, `AppID`, `OperatorID`, `UserID`, `FromMode`, `FromId`, `UserKind`, `RegistDate`, `OrderType`, `BankID`, `PaytypeID`, `PaytypeChannel`, `CurrencyID`, `RecDate`, `PlanAmount`, `AppRequestUUID`, `AppOrderId`, `AppReqComment`, `AppRequestTime`, `AccName`, `AccNumber`, `BankCode`, `IsFirstCashOfDay`, `Meta`, `Status`, `UserIP`, `OwnOrderId`, `TransMoney`, `OrderMoney`, `BankResponseTime`, `BankOrderId`, `BankCallbackTime`, `BankTime`, `BrCode`, `QrCode`, `Amount`, `OwnFee`, `UserFee`, `UserMoney`, `EndBalance`, `AmountBonus`, `EndBonus`, `SettlTable`, `SettlId`, `SettlAmount`, `SettlStatus`, `CompleteFlag`, `ActivityIds`, `IsAddBalance`, `IsAuditOrder`, `CountryID`) VALUES ";
			var sql = new StringBuilder();
	        foreach (var item in items)
			{
				count++;
				sql.Append($"('{item.OrderID}','{item.ProviderID}','{item.AppID}','{item.OperatorID}','{item.UserID}',{item.FromMode},'{item.FromId}',{item.UserKind},'{item.RegistDate?.ToString("yyyy-MM-dd HH:mm:ss")}',{item.OrderType},'{item.BankID}',{item.PaytypeID},{item.PaytypeChannel},'{item.CurrencyID}','{item.RecDate.ToString("yyyy-MM-dd HH:mm:ss")}',{item.PlanAmount},'{item.AppRequestUUID}','{item.AppOrderId}','{item.AppReqComment}','{item.AppRequestTime.ToString("yyyy-MM-dd HH:mm:ss")}','{item.AccName}','{item.AccNumber}','{item.BankCode}',{item.IsFirstCashOfDay},'{item.Meta}',{item.Status},'{item.UserIP}','{item.OwnOrderId}',{item.TransMoney},{item.OrderMoney},'{item.BankResponseTime?.ToString("yyyy-MM-dd HH:mm:ss")}','{item.BankOrderId}','{item.BankCallbackTime?.ToString("yyyy-MM-dd HH:mm:ss")}','{item.BankTime?.ToString("yyyy-MM-dd HH:mm:ss")}','{item.BrCode}','{item.QrCode}',{item.Amount},{item.OwnFee},{item.UserFee},{item.UserMoney},{item.EndBalance},{item.AmountBonus},{item.EndBonus},'{item.SettlTable}','{item.SettlId}',{item.SettlAmount},{item.SettlStatus},{item.CompleteFlag},'{item.ActivityIds}',{item.IsAddBalance},{item.IsAuditOrder},'{item.CountryID}'),");
				if (count == batchCount)
				{
					count = 0;
