...
namespace UGame.Lobby.Repository.ing
{
	...
	/// <summary>
	/// 幸运轮盘抽奖概率
	/// 【表 sa_roulette_odds 的实体类】
	/// </summary>
	[DataContract]
	[Obsolete]
	public class Sa_roulette_oddsEO : IRowMapper<Sa_roulette_oddsEO>
	{
		/// <summary>
		/// 构造函数 
		/// </summary>
		public Sa_roulette_oddsEO()
		{
			this.RaffleType = 0;
			this.UserType = 0;
			this.Level = 0;
		...
		/// <return>实体对象</return>
		public Sa_roulette_oddsEO MapRow(IDataReader reader)
		{
		...
		/// <param name = "reader">只进结果集流</param>
		/// <return>实体对象</return>
		public static Sa_roulette_oddsEO MapDataReader(IDataReader reader)
		{
		    Sa_roulette_oddsEO ret = new Sa_roulette_oddsEO();
	...
	/// <summary>
	/// 幸运轮盘抽奖概率
	/// 【表 sa_roulette_odds 的操作类】
	/// </summary>
	[Obsolete]
	public class Sa_roulette_oddsMO : MySqlTableMO<Sa_roulette_oddsEO>
	{
		/// <summary>
		/// 表名
		/// </summary>
	    public override string TableName { get; set; } = "`sa_roulette_odds`";
		...
		public Sa_roulette_oddsMO(MySqlDatabase database) : base(database) { }
		/// <summary>
		/// 构造函数
		/// </summary>
		/// <param name = "connectionStringName">配置文件.config中定义的连接字符串名称</param>
		public Sa_roulette_oddsMO(string connectionStringName = null) : base(connectionStringName) { }
	    /// <summary>
	    /// 构造函数
	    /// </summary>
	    /// <param name="connectionString">数据库连接字符串，如server=192.168.1.1;database=testdb;uid=root;pwd=root</param>
	    /// <param name="commandTimeout">CommandTimeout时间</param>
	    public Sa_roulette_oddsMO(string connectionString, int commandTimeout) : base(connectionString, commandTimeout) { }
		...
		public override int Add(Sa_roulette_oddsEO item, TransactionManager tm_ = null, bool useIgnore_ = false)
		{
			RepairAddData(item, useIgnore_, out string sql_, out List<MySqlParameter> paras_);
			return Database.ExecSqlNonQuery(sql_, paras_, tm_); 
		}
		public override async Task<int> AddAsync(Sa_roulette_oddsEO item, TransactionManager tm_ = null, bool useIgnore_ = false)
		{
			RepairAddData(item, useIgnore_, out string sql_, out List<MySqlParameter> paras_);
			return await Database.ExecSqlNonQueryAsync(sql_, paras_, tm_); 
		}
	    private void RepairAddData(Sa_roulette_oddsEO item, bool useIgnore_, out string sql_, out List<MySqlParameter> paras_)
		{
			sql_ = useIgnore_ ? "INSERT IGNORE" : "INSERT";
			sql_ += $" INTO {TableName} (`OddsID`, `OperatorID`, `CountryID`, `CurrencyID`, `RaffleType`, `UserType`, `Level`, `RewardType`, `PreRewardType`, `RewardTypeDesc`, `Position`, `Odds`, `Multiple`, `OrderNum`, `RecDate`) VALUE (@OddsID, @OperatorID, @CountryID, @CurrencyID, @RaffleType, @UserType, @Level, @RewardType, @PreRewardType, @RewardTypeDesc, @Position, @Odds, @Multiple, @OrderNum, @RecDate);";
		...
		public int AddByBatch(IEnumerable<Sa_roulette_oddsEO> items, int batchCount, TransactionManager tm_ = null)
		{
			var ret = 0;
	    ...
	    public async Task<int> AddByBatchAsync(IEnumerable<Sa_roulette_oddsEO> items, int batchCount, TransactionManager tm_ = null)
	    {
	        var ret = 0;
	        foreach (var sql in BuildAddBatchSql(items, batchCount))
	        {
	    ...
	    private IEnumerable<string> BuildAddBatchSql(IEnumerable<Sa_roulette_oddsEO> items, int batchCount)
		{
			var count = 0;
	        var insertSql = $"INSERT INTO {TableName} (`OddsID`, `OperatorID`, `CountryID`, `CurrencyID`, `RaffleType`, `UserType`, `Level`, `RewardType`, `PreRewardType`, `RewardTypeDesc`, `Position`, `Odds`, `Multiple`, `OrderNum`, `RecDate`) VALUES ";
			var sql = new StringBuilder();
	        foreach (var item in items)
			{
				count++;
				sql.Append($"('{item.OddsID}','{item.OperatorID}','{item.CountryID}','{item.CurrencyID}',{item.RaffleType},{item.UserType},{item.Level},{item.RewardType},{item.PreRewardType},'{item.RewardTypeDesc}',{item.Position},{item.Odds},{item.Multiple},{item.OrderNum},'{item.RecDate.ToString("yyyy-MM-dd HH:mm:ss")}'),");
				if (count == batchCount)
				{
					count = 0;
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>受影响的行数</return>
		public int Remove(Sa_roulette_oddsEO item, TransactionManager tm_ = null)
		{
			return RemoveByPK(item.OddsID, tm_);
		}
		public async Task<int> RemoveAsync(Sa_roulette_oddsEO item, TransactionManager tm_ = null)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>受影响的行数</return>
		public int Put(Sa_roulette_oddsEO item, TransactionManager tm_ = null)
		{
			RepairPutData(item, out string sql_, out List<MySqlParameter> paras_);
			return Database.ExecSqlNonQuery(sql_, paras_, tm_);
		}
		public async Task<int> PutAsync(Sa_roulette_oddsEO item, TransactionManager tm_ = null)
		{
			RepairPutData(item, out string sql_, out List<MySqlParameter> paras_);
			return await Database.ExecSqlNonQueryAsync(sql_, paras_, tm_);
		}
		private void RepairPutData(Sa_roulette_oddsEO item, out string sql_, out List<MySqlParameter> paras_)
		{
			sql_ = $"UPDATE {TableName} SET `OddsID` = @OddsID, `OperatorID` = @OperatorID, `CountryID` = @CountryID, `CurrencyID` = @CurrencyID, `RaffleType` = @RaffleType, `UserType` = @UserType, `Level` = @Level, `RewardType` = @RewardType, `PreRewardType` = @PreRewardType, `RewardTypeDesc` = @RewardTypeDesc, `Position` = @Position, `Odds` = @Odds, `Multiple` = @Multiple, `OrderNum` = @OrderNum WHERE `OddsID` = @OddsID_Original";
			paras_ = new List<MySqlParameter>() 
			{
				Database.CreateInParameter("@OddsID", item.OddsID, MySqlDbType.VarChar),
				Database.CreateInParameter("@OperatorID", item.OperatorID, MySqlDbType.VarChar),
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>受影响的行数</return>
		public int Put(IEnumerable<Sa_roulette_oddsEO> items, TransactionManager tm_ = null)
		{
			int ret = 0;
			foreach (var item in items)
			{
		...
		public async Task<int> PutAsync(IEnumerable<Sa_roulette_oddsEO> items, TransactionManager tm_ = null)
		{
			int ret = 0;
			foreach (var item in items)
			{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>受影响的行数</return>
		public int PutRaffleTypeByPK(string oddsID, int raffleType, TransactionManager tm_ = null)
		{
			RepairPutRaffleTypeByPKData(oddsID, raffleType, out string sql_, out List<MySqlParameter> paras_);
			return Database.ExecSqlNonQuery(sql_, paras_, tm_);
		}
		public async Task<int> PutRaffleTypeByPKAsync(string oddsID, int raffleType, TransactionManager tm_ = null)
		{
			RepairPutRaffleTypeByPKData(oddsID, raffleType, out string sql_, out List<MySqlParameter> paras_);
			return await Database.ExecSqlNonQueryAsync(sql_, paras_, tm_);
		}
		private void RepairPutRaffleTypeByPKData(string oddsID, int raffleType, out string sql_, out List<MySqlParameter> paras_)
		{
			sql_ = $"UPDATE {TableName} SET `RaffleType` = @RaffleType  WHERE `OddsID` = @OddsID";
			paras_ = new List<MySqlParameter>() 
			{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>受影响的行数</return>
		public int PutUserTypeByPK(string oddsID, int userType, TransactionManager tm_ = null)
		{
			RepairPutUserTypeByPKData(oddsID, userType, out string sql_, out List<MySqlParameter> paras_);
			return Database.ExecSqlNonQuery(sql_, paras_, tm_);
		}
		public async Task<int> PutUserTypeByPKAsync(string oddsID, int userType, TransactionManager tm_ = null)
		{
			RepairPutUserTypeByPKData(oddsID, userType, out string sql_, out List<MySqlParameter> paras_);
			return await Database.ExecSqlNonQueryAsync(sql_, paras_, tm_);
		}
		private void RepairPutUserTypeByPKData(string oddsID, int userType, out string sql_, out List<MySqlParameter> paras_)
		{
			sql_ = $"UPDATE {TableName} SET `UserType` = @UserType  WHERE `OddsID` = @OddsID";
			paras_ = new List<MySqlParameter>() 
			{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>受影响的行数</return>
		public int PutLevelByPK(string oddsID, int level, TransactionManager tm_ = null)
		{
			RepairPutLevelByPKData(oddsID, level, out string sql_, out List<MySqlParameter> paras_);
			return Database.ExecSqlNonQuery(sql_, paras_, tm_);
		}
		public async Task<int> PutLevelByPKAsync(string oddsID, int level, TransactionManager tm_ = null)
		{
			RepairPutLevelByPKData(oddsID, level, out string sql_, out List<MySqlParameter> paras_);
			return await Database.ExecSqlNonQueryAsync(sql_, paras_, tm_);
		}
		private void RepairPutLevelByPKData(string oddsID, int level, out string sql_, out List<MySqlParameter> paras_)
		{
			sql_ = $"UPDATE {TableName} SET `Level` = @Level  WHERE `OddsID` = @OddsID";
			paras_ = new List<MySqlParameter>() 
			{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>受影响的行数</return>
		public int PutPreRewardTypeByPK(string oddsID, int preRewardType, TransactionManager tm_ = null)
		{
			RepairPutPreRewardTypeByPKData(oddsID, preRewardType, out string sql_, out List<MySqlParameter> paras_);
			return Database.ExecSqlNonQuery(sql_, paras_, tm_);
		}
		public async Task<int> PutPreRewardTypeByPKAsync(string oddsID, int preRewardType, TransactionManager tm_ = null)
		{
			RepairPutPreRewardTypeByPKData(oddsID, preRewardType, out string sql_, out List<MySqlParameter> paras_);
			return await Database.ExecSqlNonQueryAsync(sql_, paras_, tm_);
		}
		private void RepairPutPreRewardTypeByPKData(string oddsID, int preRewardType, out string sql_, out List<MySqlParameter> paras_)
		{
			sql_ = $"UPDATE {TableName} SET `PreRewardType` = @PreRewardType  WHERE `OddsID` = @OddsID";
			paras_ = new List<MySqlParameter>() 
			{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>受影响的行数</return>
		public int PutMultipleByPK(string oddsID, float multiple, TransactionManager tm_ = null)
		{
			RepairPutMultipleByPKData(oddsID, multiple, out string sql_, out List<MySqlParameter> paras_);
			return Database.ExecSqlNonQuery(sql_, paras_, tm_);
		}
		public async Task<int> PutMultipleByPKAsync(string oddsID, float multiple, TransactionManager tm_ = null)
		{
			RepairPutMultipleByPKData(oddsID, multiple, out string sql_, out List<MySqlParameter> paras_);
			return await Database.ExecSqlNonQueryAsync(sql_, paras_, tm_);
		}
		private void RepairPutMultipleByPKData(string oddsID, float multiple, out string sql_, out List<MySqlParameter> paras_)
		{
			sql_ = $"UPDATE {TableName} SET `Multiple` = @Multiple  WHERE `OddsID` = @OddsID";
			paras_ = new List<MySqlParameter>() 
			{
		...
		public bool Set(Sa_roulette_oddsEO item, TransactionManager tm = null)
		{
			bool ret = true;
		...
		public async Task<bool> SetAsync(Sa_roulette_oddsEO item, TransactionManager tm = null)
		{
			bool ret = true;
			if(GetByPK(item.OddsID) == null)
			{
		...
		/// <return></return>
		public Sa_roulette_oddsEO GetByPK(string oddsID, TransactionManager tm_ = null, bool isForUpdate_ = false)
		{
			RepairGetByPKData(oddsID, out string sql_, out List<MySqlParameter> paras_, isForUpdate_, tm_);
			return Database.ExecSqlSingle(sql_, paras_, tm_, Sa_roulette_oddsEO.MapDataReader);
		}
		public async Task<Sa_roulette_oddsEO> GetByPKAsync(string oddsID, TransactionManager tm_ = null, bool isForUpdate_ = false)
		{
			RepairGetByPKData(oddsID, out string sql_, out List<MySqlParameter> paras_, isForUpdate_, tm_);
			return await Database.ExecSqlSingleAsync(sql_, paras_, tm_, Sa_roulette_oddsEO.MapDataReader);
		}
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return></return>
		public int GetRaffleTypeByPK(string oddsID, TransactionManager tm_ = null)
		{
			var paras_ = new List<MySqlParameter>() 
			{
				Database.CreateInParameter("@OddsID", oddsID, MySqlDbType.VarChar),
			};
			return (int)GetScalar("`RaffleType`", "`OddsID` = @OddsID", paras_, tm_);
		}
		public async Task<int> GetRaffleTypeByPKAsync(string oddsID, TransactionManager tm_ = null)
		{
			var paras_ = new List<MySqlParameter>() 
			{
				Database.CreateInParameter("@OddsID", oddsID, MySqlDbType.VarChar),
			};
			return (int)await GetScalarAsync("`RaffleType`", "`OddsID` = @OddsID", paras_, tm_);
		}
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return></return>
		public int GetUserTypeByPK(string oddsID, TransactionManager tm_ = null)
		{
			var paras_ = new List<MySqlParameter>() 
			{
				Database.CreateInParameter("@OddsID", oddsID, MySqlDbType.VarChar),
			};
			return (int)GetScalar("`UserType`", "`OddsID` = @OddsID", paras_, tm_);
		}
		public async Task<int> GetUserTypeByPKAsync(string oddsID, TransactionManager tm_ = null)
		{
			var paras_ = new List<MySqlParameter>() 
			{
				Database.CreateInParameter("@OddsID", oddsID, MySqlDbType.VarChar),
			};
			return (int)await GetScalarAsync("`UserType`", "`OddsID` = @OddsID", paras_, tm_);
		}
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return></return>
		public int GetLevelByPK(string oddsID, TransactionManager tm_ = null)
		{
			var paras_ = new List<MySqlParameter>() 
			{
				Database.CreateInParameter("@OddsID", oddsID, MySqlDbType.VarChar),
			};
			return (int)GetScalar("`Level`", "`OddsID` = @OddsID", paras_, tm_);
		}
		public async Task<int> GetLevelByPKAsync(string oddsID, TransactionManager tm_ = null)
		{
			var paras_ = new List<MySqlParameter>() 
			{
				Database.CreateInParameter("@OddsID", oddsID, MySqlDbType.VarChar),
			};
			return (int)await GetScalarAsync("`Level`", "`OddsID` = @OddsID", paras_, tm_);
		}
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return></return>
		public int GetPreRewardTypeByPK(string oddsID, TransactionManager tm_ = null)
		{
			var paras_ = new List<MySqlParameter>() 
			{
				Database.CreateInParameter("@OddsID", oddsID, MySqlDbType.VarChar),
			};
			return (int)GetScalar("`PreRewardType`", "`OddsID` = @OddsID", paras_, tm_);
		}
		public async Task<int> GetPreRewardTypeByPKAsync(string oddsID, TransactionManager tm_ = null)
		{
			var paras_ = new List<MySqlParameter>() 
			{
				Database.CreateInParameter("@OddsID", oddsID, MySqlDbType.VarChar),
			};
			return (int)await GetScalarAsync("`PreRewardType`", "`OddsID` = @OddsID", paras_, tm_);
		}
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return></return>
		public float GetMultipleByPK(string oddsID, TransactionManager tm_ = null)
		{
			var paras_ = new List<MySqlParameter>() 
			{
				Database.CreateInParameter("@OddsID", oddsID, MySqlDbType.VarChar),
			};
			return (float)GetScalar("`Multiple`", "`OddsID` = @OddsID", paras_, tm_);
		}
		public async Task<float> GetMultipleByPKAsync(string oddsID, TransactionManager tm_ = null)
		{
			var paras_ = new List<MySqlParameter>() 
			{
				Database.CreateInParameter("@OddsID", oddsID, MySqlDbType.VarChar),
			};
			return (float)await GetScalarAsync("`Multiple`", "`OddsID` = @OddsID", paras_, tm_);
		}
		...
		public List<Sa_roulette_oddsEO> GetByOperatorID(string operatorID)
		{
			return GetByOperatorID(operatorID, 0, string.Empty, null);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByOperatorIDAsync(string operatorID)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByOperatorID(string operatorID, TransactionManager tm_)
		{
			return GetByOperatorID(operatorID, 0, string.Empty, tm_);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByOperatorIDAsync(string operatorID, TransactionManager tm_)
		{
		...
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByOperatorID(string operatorID, int top_)
		{
			return GetByOperatorID(operatorID, top_, string.Empty, null);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByOperatorIDAsync(string operatorID, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByOperatorID(string operatorID, int top_, TransactionManager tm_)
		{
			return GetByOperatorID(operatorID, top_, string.Empty, tm_);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByOperatorIDAsync(string operatorID, int top_, TransactionManager tm_)
		{
		...
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByOperatorID(string operatorID, string sort_)
		{
			return GetByOperatorID(operatorID, 0, sort_, null);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByOperatorIDAsync(string operatorID, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByOperatorID(string operatorID, string sort_, TransactionManager tm_)
		{
			return GetByOperatorID(operatorID, 0, sort_, tm_);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByOperatorIDAsync(string operatorID, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByOperatorID(string operatorID, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`OperatorID` = @OperatorID", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@OperatorID", operatorID, MySqlDbType.VarChar));
			return Database.ExecSqlList(sql_, paras_, tm_, Sa_roulette_oddsEO.MapDataReader);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByOperatorIDAsync(string operatorID, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`OperatorID` = @OperatorID", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@OperatorID", operatorID, MySqlDbType.VarChar));
			return await Database.ExecSqlListAsync(sql_, paras_, tm_, Sa_roulette_oddsEO.MapDataReader);
		}
		...
		/// /// <param name = "countryID">国家编码ISO 3166-1三位字母</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByCountryID(string countryID)
		{
			return GetByCountryID(countryID, 0, string.Empty, null);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByCountryIDAsync(string countryID)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByCountryID(string countryID, TransactionManager tm_)
		{
			return GetByCountryID(countryID, 0, string.Empty, tm_);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByCountryIDAsync(string countryID, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByCountryID(string countryID, int top_)
		{
			return GetByCountryID(countryID, top_, string.Empty, null);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByCountryIDAsync(string countryID, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByCountryID(string countryID, int top_, TransactionManager tm_)
		{
			return GetByCountryID(countryID, top_, string.Empty, tm_);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByCountryIDAsync(string countryID, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByCountryID(string countryID, string sort_)
		{
			return GetByCountryID(countryID, 0, sort_, null);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByCountryIDAsync(string countryID, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByCountryID(string countryID, string sort_, TransactionManager tm_)
		{
			return GetByCountryID(countryID, 0, sort_, tm_);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByCountryIDAsync(string countryID, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByCountryID(string countryID, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Sa_roulette_oddsEO>> GetByCountryIDAsync(string countryID, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`CountryID` = @CountryID", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@CountryID", countryID, MySqlDbType.VarChar));
		...
		/// /// <param name = "currencyID">货币类型</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByCurrencyID(string currencyID)
		{
			return GetByCurrencyID(currencyID, 0, string.Empty, null);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByCurrencyIDAsync(string currencyID)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByCurrencyID(string currencyID, TransactionManager tm_)
		{
			return GetByCurrencyID(currencyID, 0, string.Empty, tm_);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByCurrencyIDAsync(string currencyID, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByCurrencyID(string currencyID, int top_)
		{
			return GetByCurrencyID(currencyID, top_, string.Empty, null);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByCurrencyIDAsync(string currencyID, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByCurrencyID(string currencyID, int top_, TransactionManager tm_)
		{
			return GetByCurrencyID(currencyID, top_, string.Empty, tm_);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByCurrencyIDAsync(string currencyID, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByCurrencyID(string currencyID, string sort_)
		{
			return GetByCurrencyID(currencyID, 0, sort_, null);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByCurrencyIDAsync(string currencyID, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByCurrencyID(string currencyID, string sort_, TransactionManager tm_)
		{
			return GetByCurrencyID(currencyID, 0, sort_, tm_);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByCurrencyIDAsync(string currencyID, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByCurrencyID(string currencyID, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Sa_roulette_oddsEO>> GetByCurrencyIDAsync(string currencyID, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`CurrencyID` = @CurrencyID", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@CurrencyID", currencyID, MySqlDbType.VarChar));
		...
		/// /// <param name = "raffleType">抽奖类型1-freespin2-真金</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByRaffleType(int raffleType)
		{
			return GetByRaffleType(raffleType, 0, string.Empty, null);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByRaffleTypeAsync(int raffleType)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByRaffleType(int raffleType, TransactionManager tm_)
		{
			return GetByRaffleType(raffleType, 0, string.Empty, tm_);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByRaffleTypeAsync(int raffleType, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByRaffleType(int raffleType, int top_)
		{
			return GetByRaffleType(raffleType, top_, string.Empty, null);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByRaffleTypeAsync(int raffleType, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByRaffleType(int raffleType, int top_, TransactionManager tm_)
		{
			return GetByRaffleType(raffleType, top_, string.Empty, tm_);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByRaffleTypeAsync(int raffleType, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByRaffleType(int raffleType, string sort_)
		{
			return GetByRaffleType(raffleType, 0, sort_, null);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByRaffleTypeAsync(int raffleType, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByRaffleType(int raffleType, string sort_, TransactionManager tm_)
		{
			return GetByRaffleType(raffleType, 0, sort_, tm_);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByRaffleTypeAsync(int raffleType, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByRaffleType(int raffleType, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Sa_roulette_oddsEO>> GetByRaffleTypeAsync(int raffleType, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`RaffleType` = @RaffleType", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@RaffleType", raffleType, MySqlDbType.Int32));
		...
		/// /// <param name = "userType">抽奖用户类型1-新用户2-盈利用户3-亏损用户</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByUserType(int userType)
		{
			return GetByUserType(userType, 0, string.Empty, null);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByUserTypeAsync(int userType)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByUserType(int userType, TransactionManager tm_)
		{
			return GetByUserType(userType, 0, string.Empty, tm_);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByUserTypeAsync(int userType, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByUserType(int userType, int top_)
		{
			return GetByUserType(userType, top_, string.Empty, null);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByUserTypeAsync(int userType, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByUserType(int userType, int top_, TransactionManager tm_)
		{
			return GetByUserType(userType, top_, string.Empty, tm_);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByUserTypeAsync(int userType, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByUserType(int userType, string sort_)
		{
			return GetByUserType(userType, 0, sort_, null);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByUserTypeAsync(int userType, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByUserType(int userType, string sort_, TransactionManager tm_)
		{
			return GetByUserType(userType, 0, sort_, tm_);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByUserTypeAsync(int userType, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByUserType(int userType, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Sa_roulette_oddsEO>> GetByUserTypeAsync(int userType, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`UserType` = @UserType", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@UserType", userType, MySqlDbType.Int32));
		...
		/// /// <param name = "level">抽奖等级1-基础奖励2-二级奖励</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByLevel(int level)
		{
			return GetByLevel(level, 0, string.Empty, null);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByLevelAsync(int level)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByLevel(int level, TransactionManager tm_)
		{
			return GetByLevel(level, 0, string.Empty, tm_);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByLevelAsync(int level, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByLevel(int level, int top_)
		{
			return GetByLevel(level, top_, string.Empty, null);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByLevelAsync(int level, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByLevel(int level, int top_, TransactionManager tm_)
		{
			return GetByLevel(level, top_, string.Empty, tm_);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByLevelAsync(int level, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByLevel(int level, string sort_)
		{
			return GetByLevel(level, 0, sort_, null);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByLevelAsync(int level, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByLevel(int level, string sort_, TransactionManager tm_)
		{
			return GetByLevel(level, 0, sort_, tm_);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByLevelAsync(int level, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByLevel(int level, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Sa_roulette_oddsEO>> GetByLevelAsync(int level, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`Level` = @Level", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@Level", level, MySqlDbType.Int32));
		...
		/// /// <param name = "rewardType">奖励类型1-无奖励2-真金3-二级抽奖A4-二级抽奖B</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByRewardType(int rewardType)
		{
			return GetByRewardType(rewardType, 0, string.Empty, null);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByRewardTypeAsync(int rewardType)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByRewardType(int rewardType, TransactionManager tm_)
		{
			return GetByRewardType(rewardType, 0, string.Empty, tm_);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByRewardTypeAsync(int rewardType, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByRewardType(int rewardType, int top_)
		{
			return GetByRewardType(rewardType, top_, string.Empty, null);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByRewardTypeAsync(int rewardType, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByRewardType(int rewardType, int top_, TransactionManager tm_)
		{
			return GetByRewardType(rewardType, top_, string.Empty, tm_);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByRewardTypeAsync(int rewardType, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByRewardType(int rewardType, string sort_)
		{
			return GetByRewardType(rewardType, 0, sort_, null);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByRewardTypeAsync(int rewardType, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByRewardType(int rewardType, string sort_, TransactionManager tm_)
		{
			return GetByRewardType(rewardType, 0, sort_, tm_);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByRewardTypeAsync(int rewardType, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByRewardType(int rewardType, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Sa_roulette_oddsEO>> GetByRewardTypeAsync(int rewardType, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`RewardType` = @RewardType", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@RewardType", rewardType, MySqlDbType.Int32));
		...
		/// /// <param name = "preRewardType">上一级奖励类型</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByPreRewardType(int preRewardType)
		{
			return GetByPreRewardType(preRewardType, 0, string.Empty, null);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByPreRewardTypeAsync(int preRewardType)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByPreRewardType(int preRewardType, TransactionManager tm_)
		{
			return GetByPreRewardType(preRewardType, 0, string.Empty, tm_);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByPreRewardTypeAsync(int preRewardType, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByPreRewardType(int preRewardType, int top_)
		{
			return GetByPreRewardType(preRewardType, top_, string.Empty, null);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByPreRewardTypeAsync(int preRewardType, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByPreRewardType(int preRewardType, int top_, TransactionManager tm_)
		{
			return GetByPreRewardType(preRewardType, top_, string.Empty, tm_);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByPreRewardTypeAsync(int preRewardType, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByPreRewardType(int preRewardType, string sort_)
		{
			return GetByPreRewardType(preRewardType, 0, sort_, null);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByPreRewardTypeAsync(int preRewardType, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByPreRewardType(int preRewardType, string sort_, TransactionManager tm_)
		{
			return GetByPreRewardType(preRewardType, 0, sort_, tm_);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByPreRewardTypeAsync(int preRewardType, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByPreRewardType(int preRewardType, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Sa_roulette_oddsEO>> GetByPreRewardTypeAsync(int preRewardType, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`PreRewardType` = @PreRewardType", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@PreRewardType", preRewardType, MySqlDbType.Int32));
		...
		/// /// <param name = "rewardTypeDesc">奖励描述</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByRewardTypeDesc(string rewardTypeDesc)
		{
			return GetByRewardTypeDesc(rewardTypeDesc, 0, string.Empty, null);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByRewardTypeDescAsync(string rewardTypeDesc)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByRewardTypeDesc(string rewardTypeDesc, TransactionManager tm_)
		{
			return GetByRewardTypeDesc(rewardTypeDesc, 0, string.Empty, tm_);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByRewardTypeDescAsync(string rewardTypeDesc, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByRewardTypeDesc(string rewardTypeDesc, int top_)
		{
			return GetByRewardTypeDesc(rewardTypeDesc, top_, string.Empty, null);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByRewardTypeDescAsync(string rewardTypeDesc, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByRewardTypeDesc(string rewardTypeDesc, int top_, TransactionManager tm_)
		{
			return GetByRewardTypeDesc(rewardTypeDesc, top_, string.Empty, tm_);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByRewardTypeDescAsync(string rewardTypeDesc, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByRewardTypeDesc(string rewardTypeDesc, string sort_)
		{
			return GetByRewardTypeDesc(rewardTypeDesc, 0, sort_, null);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByRewardTypeDescAsync(string rewardTypeDesc, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByRewardTypeDesc(string rewardTypeDesc, string sort_, TransactionManager tm_)
		{
			return GetByRewardTypeDesc(rewardTypeDesc, 0, sort_, tm_);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByRewardTypeDescAsync(string rewardTypeDesc, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByRewardTypeDesc(string rewardTypeDesc, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Sa_roulette_oddsEO>> GetByRewardTypeDescAsync(string rewardTypeDesc, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL(rewardTypeDesc != null ? "`RewardTypeDesc` = @RewardTypeDesc" : "`RewardTypeDesc` IS NULL", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			if (rewardTypeDesc != null)
		...
		/// /// <param name = "position">位置</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByPosition(int position)
		{
			return GetByPosition(position, 0, string.Empty, null);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByPositionAsync(int position)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByPosition(int position, TransactionManager tm_)
		{
			return GetByPosition(position, 0, string.Empty, tm_);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByPositionAsync(int position, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByPosition(int position, int top_)
		{
			return GetByPosition(position, top_, string.Empty, null);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByPositionAsync(int position, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByPosition(int position, int top_, TransactionManager tm_)
		{
			return GetByPosition(position, top_, string.Empty, tm_);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByPositionAsync(int position, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByPosition(int position, string sort_)
		{
			return GetByPosition(position, 0, sort_, null);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByPositionAsync(int position, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByPosition(int position, string sort_, TransactionManager tm_)
		{
			return GetByPosition(position, 0, sort_, tm_);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByPositionAsync(int position, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByPosition(int position, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Sa_roulette_oddsEO>> GetByPositionAsync(int position, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`Position` = @Position", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@Position", position, MySqlDbType.Int32));
		...
		/// /// <param name = "odds">概率</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByOdds(int odds)
		{
			return GetByOdds(odds, 0, string.Empty, null);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByOddsAsync(int odds)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByOdds(int odds, TransactionManager tm_)
		{
			return GetByOdds(odds, 0, string.Empty, tm_);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByOddsAsync(int odds, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByOdds(int odds, int top_)
		{
			return GetByOdds(odds, top_, string.Empty, null);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByOddsAsync(int odds, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByOdds(int odds, int top_, TransactionManager tm_)
		{
			return GetByOdds(odds, top_, string.Empty, tm_);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByOddsAsync(int odds, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByOdds(int odds, string sort_)
		{
			return GetByOdds(odds, 0, sort_, null);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByOddsAsync(int odds, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByOdds(int odds, string sort_, TransactionManager tm_)
		{
			return GetByOdds(odds, 0, sort_, tm_);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByOddsAsync(int odds, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByOdds(int odds, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Sa_roulette_oddsEO>> GetByOddsAsync(int odds, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`Odds` = @Odds", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@Odds", odds, MySqlDbType.Int32));
		...
		/// /// <param name = "multiple">倍率</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByMultiple(float multiple)
		{
			return GetByMultiple(multiple, 0, string.Empty, null);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByMultipleAsync(float multiple)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByMultiple(float multiple, TransactionManager tm_)
		{
			return GetByMultiple(multiple, 0, string.Empty, tm_);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByMultipleAsync(float multiple, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByMultiple(float multiple, int top_)
		{
			return GetByMultiple(multiple, top_, string.Empty, null);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByMultipleAsync(float multiple, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByMultiple(float multiple, int top_, TransactionManager tm_)
		{
			return GetByMultiple(multiple, top_, string.Empty, tm_);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByMultipleAsync(float multiple, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByMultiple(float multiple, string sort_)
		{
			return GetByMultiple(multiple, 0, sort_, null);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByMultipleAsync(float multiple, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByMultiple(float multiple, string sort_, TransactionManager tm_)
		{
			return GetByMultiple(multiple, 0, sort_, tm_);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByMultipleAsync(float multiple, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByMultiple(float multiple, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Sa_roulette_oddsEO>> GetByMultipleAsync(float multiple, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`Multiple` = @Multiple", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@Multiple", multiple, MySqlDbType.Float));
		...
		/// /// <param name = "orderNum">排序（从小到大）</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByOrderNum(int orderNum)
		{
			return GetByOrderNum(orderNum, 0, string.Empty, null);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByOrderNumAsync(int orderNum)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByOrderNum(int orderNum, TransactionManager tm_)
		{
			return GetByOrderNum(orderNum, 0, string.Empty, tm_);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByOrderNumAsync(int orderNum, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByOrderNum(int orderNum, int top_)
		{
			return GetByOrderNum(orderNum, top_, string.Empty, null);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByOrderNumAsync(int orderNum, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByOrderNum(int orderNum, int top_, TransactionManager tm_)
		{
			return GetByOrderNum(orderNum, top_, string.Empty, tm_);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByOrderNumAsync(int orderNum, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByOrderNum(int orderNum, string sort_)
		{
			return GetByOrderNum(orderNum, 0, sort_, null);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByOrderNumAsync(int orderNum, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByOrderNum(int orderNum, string sort_, TransactionManager tm_)
		{
			return GetByOrderNum(orderNum, 0, sort_, tm_);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByOrderNumAsync(int orderNum, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByOrderNum(int orderNum, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Sa_roulette_oddsEO>> GetByOrderNumAsync(int orderNum, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`OrderNum` = @OrderNum", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@OrderNum", orderNum, MySqlDbType.Int32));
		...
		/// /// <param name = "recDate">记录时间</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByRecDate(DateTime recDate)
		{
			return GetByRecDate(recDate, 0, string.Empty, null);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByRecDateAsync(DateTime recDate)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByRecDate(DateTime recDate, TransactionManager tm_)
		{
			return GetByRecDate(recDate, 0, string.Empty, tm_);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByRecDateAsync(DateTime recDate, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByRecDate(DateTime recDate, int top_)
		{
			return GetByRecDate(recDate, top_, string.Empty, null);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByRecDateAsync(DateTime recDate, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByRecDate(DateTime recDate, int top_, TransactionManager tm_)
		{
			return GetByRecDate(recDate, top_, string.Empty, tm_);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByRecDateAsync(DateTime recDate, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByRecDate(DateTime recDate, string sort_)
		{
			return GetByRecDate(recDate, 0, sort_, null);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByRecDateAsync(DateTime recDate, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByRecDate(DateTime recDate, string sort_, TransactionManager tm_)
		{
			return GetByRecDate(recDate, 0, sort_, tm_);
		}
		public async Task<List<Sa_roulette_oddsEO>> GetByRecDateAsync(DateTime recDate, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<Sa_roulette_oddsEO> GetByRecDate(DateTime recDate, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<Sa_roulette_oddsEO>> GetByRecDateAsync(DateTime recDate, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`RecDate` = @RecDate", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@RecDate", recDate, MySqlDbType.DateTime));
