...
namespace UGame.Lobby.Service.Services.Cash
{
    ...
    public class CashServiceFactory<T>
    {
        ...
        public async Task CheckCashCondition(T t)
        {
            ...
            if (ipo.UserExEo.TotalCashCount == 0)
            {
                ...
                if (ipo.Amount < cashConfig.FirstCashMinAmount.AToM(ipo.CurrencyId))
                    throw new CustomException(LobbyCodes.RS_UNKNOWN, $"Minimum amount {cashConfig.CashMinAmount.AToM(ipo.CurrencyId)}.");
            }
            else
            {
                //如果提现金额小于非首次提现最小提现金额
            ...
            if (ipo.Amount > cashConfig.CashMaxAmount.AToM(ipo.CurrencyId))
                throw new CustomException(LobbyCodes.RS_UNKNOWN, $"Maximum amount {cashConfig.CashMaxAmount.AToM(ipo.CurrencyId)}.");
            ...
            if (ipo.CashConfigEo.IsVerifyMobile && string.IsNullOrWhiteSpace(ipo.UserEo.Mobile))
                throw new CustomException(LobbyCodes.RS_CASH_NOTEXISTSMOBILE, "Condition not met.1");

            //如果提现时，用户充值总额 < 提现配置的最低充值金额
            if (cashConfig.CashPayAmount > 0 && ipo.UserExEo.TotalPayAmount < cashConfig.CashPayAmount)
                throw new CustomException(LobbyCodes.RS_CASH_NOTMEETMINPAY, "Condition not met.2");

            //无可提现金额
            var drawAmount = await LobbyUtil.GetAllowCashAmount(ipo.UserId, ipo.CurrencyId);
            if (drawAmount == 0)
                throw new CustomException(LobbyCodes.RS_UNKNOWN, "Condition not met.3");
            ...
            //获取时间查询区间
            var timeInterval = LobbyUtil.GetDBTimeInterval(ipo.OperatorId);
            ...
            if (ipo.SumCashNum + 1 > ipo.CashConfigEo.DayCashNumLimit)
                throw new CustomException(LobbyCodes.RS_CASH_NUM_LIMIT, "The withdrawal frequency has reached the upper limit");
            ...
            if (ipo.SOperatorEO != null && ipo.SOperatorEO.OperatorVersion == (int)OperatorVersionsEnum.longVersion)
            {
                if (ipo.SumCashAmount + ipo.Amount.MToA(ipo.CurrencyId) > ipo.CashConfigEo.DayCashMaxLimitAmount)
                    throw new CustomException(LobbyCodes.RS_DAY_CASH_AMOUNT_LIMIT, "Exceeding the daily withdrawal limit");
            }
            ...
            if (conditionList.Any())
            {
                ...
                if (conditionConfigEo != null)
                {
                    if (ipo.UserExEo.TotalPayAmount < conditionConfigEo.TotalPayAmount)
                        throw new CustomException(LobbyCodes.RS_NOT_PAYLIMIT, "Not meeting the recharge amount.", result: (conditionConfigEo.TotalPayAmount - ipo.UserExEo.TotalPayAmount).AToM(conditionConfigEo.CurrencyID));
                }
            }
        }
    ...
    public class LongVersionService<T> : ICashService<T>
    {
        ...
        public async Task CheckCashCondition(T t)
        {
            ...
            var drawAmount = await LobbyUtil.GetAllowCashAmount(ipo.UserId, ipo.CurrencyId);
            if (drawAmount == 0)
                throw new CustomException(LobbyCodes.RS_UNKNOWN, "Condition not met.2");

            //用户当前vip等级配置
            ipo.CurrVipConfig = LobbyDbCacheUtil.GetVipConfig(ipo.OperatorId, ipo.UserEo.VIP);
            //当前提现金额大于vip等级对应的当日最大提现金额
            if (ipo.Amount > ipo.CurrVipConfig.DayCashAmountLimit.AToM(ipo.CurrencyId))
                throw new CustomException(LobbyCodes.RS_UNKNOWN, $"Maximum amount {ipo.CurrVipConfig.DayCashAmountLimit.AToM(ipo.CurrencyId)}.");
